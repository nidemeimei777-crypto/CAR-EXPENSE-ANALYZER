...
from app.models import Car, Expense, FuelUp
...
async def get_pie_chart(
    ...
    colors = ['#667eea', '#48bb78', '#ed8936', '#e53e3e', '#38a169', '#dd6b20', '#4299e1', '#9f7aea']
...
async def compare_cars(
    ...
    for car in cars:
        # Получаем расходы
        expenses_result = await db.execute(select(Expense).where(Expense.car_id == car.id))
        expenses = expenses_result.scalars().all()
        
        total_expenses = sum(e.amount for e in expenses)
        
        # Получаем заправки для расчёта расхода топлива
        fuelups_result = await db.execute(select(FuelUp).where(FuelUp.car_id == car.id).order_by(FuelUp.date))
        ...
        efficiency = None
        if avg_consumption:
            efficiency = round((total_expenses / total_distance) * 100, 2) if total_distance > 0 else None
